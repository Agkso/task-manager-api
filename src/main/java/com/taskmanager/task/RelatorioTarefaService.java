...
/**
 * Extraido de TarefaService de proposito: o proxy de cache do Spring so
 * intercepta a chamada ANTES do metodo rodar - se a checagem de membership
 * (obterMembro) estivesse dentro do mesmo metodo anotado com @Cacheable, um
 * segundo usuario (mesmo sem ser membro do projeto) poderia receber uma
 * resposta ja cacheada sem nunca passar pela checagem de acesso, porque um
 * cache hit nao executa o corpo do metodo. Isolar a agregacao aqui deixa
 * essa garantia visivel na assinatura: quem chama TarefaService.gerarRelatorio
 * e obrigado a autorizar antes de chegar aqui.
 */
@Service
public class RelatorioTarefaService {
    ...
    public RelatorioTarefaService(TarefaRepository tarefaRepository) {
